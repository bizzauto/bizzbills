...
type CreditNoteDetail = {
  ...
  status: string;
  date: string;
  invoice: { id: string; invoiceNumber: string; customerName: string; total: number } | null;
...
export default function CreditNoteDetailPage() {
  ...
  if (!note) return <main className="p-6 text-sm text-red-400">Credit note not found.</main>;
  ...
  const docData = {
    ...
    customerGstin: note.customerGstin || undefined,
      ...
      <section className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-6 shadow-2xl shadow-black/20 backdrop-blur">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold text-white">{note.creditNoteNumber}</h1>
          </div>
