...
export default function NewQuotationPage() {
  ...
  const [form, setForm] = useState({ partyId: "", partyName: "", orderDate: new Date().toISOString().split("T")[0], validUntil: "", notes: "" });
    ...
    try {
      ...
      const res = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          orderDate: new Date(form.orderDate), deliveryDate: form.validUntil ? new Date(form.validUntil) : null,
      ...
      else alert("Failed to create quotation");
    } finally { setSaving(false); }
  }

  return (
    <main className="mx-auto max-w-3xl pb-10">
      <h1 className="text-2xl font-semibold text-white mb-6">New Quotation</h1>
      <form onSubmit={handleSubmit} className="space-y-4 rounded-[1.5rem] border border-white/10 bg-slate-900/70 p-6 backdrop-blur">
        ...
        <label className="text-xs text-slate-400">Notes / Terms<textarea value={form.notes} onChange={set("notes")} rows={3} placeholder="Terms and conditions…" className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2.5 text-sm text-white outline-none placeholder:text-slate-500 focus:border-cyan-500/50" /></label>

        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={saving} className="rounded-full bg-cyan-500 px-6 py-2.5 text-sm font-semibold text-slate-950 hover:bg-cyan-400 disabled:opacity-50">{saving ? "Saving…" : "Save Quotation"}</button>
