...
const orderTypes = [
  { key: "sales_order", label: "Sales Orders" }, { key: "purchase_order", label: "Purchase Orders" },
  { key: "quotation", label: "Quotations" }, { key: "delivery_challan", label: "Delivery Challans" },
];
      ...
      <section className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-6 shadow-2xl shadow-black/20 backdrop-blur">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold text-white">{orderTypes.find((t) => t.key === tab)?.label}</h1>
          </div>
          <Link href="/orders/new" className="rounded-full bg-cyan-500 px-4 py-1.5 text-xs font-semibold text-slate-950 hover:bg-cyan-400">+ New Order</Link>
        </div>
      </section>
        ...
        {orderTypes.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`rounded-full px-4 py-1.5 text-xs font-medium transition ${
              tab === t.key ? "bg-cyan-500 text-slate-950" : "border border-white/10 text-slate-300 hover:bg-white/5"
            }`}>{t.label}</button>
        ))}
      </div>
              ...
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id} className="border-t border-white/5 hover:bg-white/5">
                    <td className="p-3 text-white font-mono"><Link href={`/orders/${o.id}`} className="hover:text-cyan-300">{o.orderNumber}</Link></td>
