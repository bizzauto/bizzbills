...
export default function NewOrderPage() {
  ...
  const [form, setForm] = useState({ orderType: "sales_order", partyId: "", partyName: "", orderDate: new Date().toISOString().split("T")[0], notes: "" });
  ...
  function updateLine(idx: number, key: string, val: string) {
    const copy = [...lines]; (copy[idx] as any)[key] = val; setLines(copy);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...form,
          orderNumber: `${form.orderType === "purchase_order" ? "PO" : form.orderType === "quotation" ? "QT" : form.orderType === "delivery_challan" ? "DC" : "SO"}-${Date.now()}`,
    ...
    <main className="mx-auto max-w-3xl pb-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-[1.5rem] border border-white/10 bg-slate-900/70 p-6 backdrop-blur">
        <div className="grid gap-4 md:grid-cols-2">
          <label className="text-xs text-slate-400">Type<select value={form.orderType} onChange={set("orderType")} className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2.5 text-sm text-white outline-none focus:border-cyan-500/50">
            <option value="sales_order">Sales Order</option><option value="purchase_order">Purchase Order</option>
            <option value="quotation">Quotation</option><option value="delivery_challan">Delivery Challan</option>
          </select></label>
        ...
        <label className="text-xs text-slate-400">Notes<textarea value={form.notes} onChange={set("notes")} rows={2} className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2.5 text-sm text-white outline-none" /></label>

        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={saving} className="rounded-full bg-cyan-500 px-6 py-2.5 text-sm font-semibold text-slate-950 hover:bg-cyan-400 disabled:opacity-50">{saving ? "Saving…" : "Save Order"}</button>
